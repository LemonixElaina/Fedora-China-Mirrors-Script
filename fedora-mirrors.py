...
print('此工具专门为 Fedora 30 以上的 Fedora 准备')
is_start = input('请确保你的 Fedora 版本在 30 以上、Python 版本在 3.0 以上 [y/N] ').lower()
...
skip_if_unavailable=False''',
    ...
    'fedora-modular.repo': '''[fedora-modular]
...
skip_if_unavailable=False''',
    ...
    'fedora-updates-modular': '''[updates-modular]
...
skip_if_unavailable=False''',
    ...
    'fedora-updates-modular.repo':  '''[updates-modular]
name=Fedora Modular $releasever - $basearch - Updates
failovermethod=priority
...
choice = int(input('输入模式 (0:切换清华源  1:恢复官方源) [   ]\b\b\b'))
mode = {
    0: tsinghua,
    1: original
}
...
try: 
    ...
    print('此工具需要读取系统文件的权限，已自动使用 root 用户运行本程序')
    system(f'sudo python3 {argv[0]}')

print('写入成功')
is_flush = input('准备刷新元数据缓存 (dnf makecache)? [y/N] ').lower()
if is_flush == 'n':
    close()
